...
var Label = function (props) {
    ...
    var box = bbox && debug ? (React__default['default'].createElement("rect", { transform: rotate, x: bbox.x - bboxOffset, y: bbox.y - bboxOffset, width: bbox.width + bboxOffset * 2, height: bbox.height + bboxOffset * 2, style: { stroke: "bisque", fill: "none" } })) : null;
    var text = (React__default['default'].createElement("text", { ref: ref, textAnchor: textAnchor, style: style, key: id, transform: rotate, className: labelClassed },
        React__default['default'].createElement("tspan", { x: labelX, y: labelY, key: "label-line" }, label)));
...
var Shape = function (props) {
    ...
    if (shape === ShapeType.Cloud) {
        ...
        shapeElement = (React__default['default'].createElement("path", { d: cloudPath, style: style.shape[styleModifier], className: shapeClasses }));
        switch (labelPosition) {
            case LabelPosition.Top:
            case LabelPosition.TopRight:
            case LabelPosition.TopLeft:
        ...
        var posy = y - radius;
        var width = 2 * radius;
        shapeElement = (React__default['default'].createElement("rect", { x: posx, y: posy, rx: rx, ry: ry, width: width, height: width, style: style.shape[styleModifier], className: shapeClasses }));
        switch (labelPosition) {
            case LabelPosition.Left:
                deltaLabelX -= 2;
                break;
        ...
        shapeElement = (React__default['default'].createElement("circle", { cx: x, cy: y, r: radius, style: style.shape[styleModifier], className: shapeClasses }));
    }
    var labelElement;
    if (label) {
        labelElement = (React__default['default'].createElement(Label, { label: label, x: x + deltaLabelX, y: y + deltaLabelX, xOffset: deltaLabelX, yOffset: deltaLabelY, labelPosition: labelPosition, labelRadialDistance: radius + 3, labelClassed: labelClasses, style: style.label[styleModifier] }));
    }
    return (React__default['default'].createElement("g", { onClick: handMouseClick, onMouseOver: handleMouseOver, onMouseDown: handleMouseDown },
        shapeElement,
        labelElement));
};
