...
test("Make a topology with the constructor", () => {
    const g = new Graph<MetaType, NodeType, EdgeType>({ name: "top_level", version: "3.1" });
...
test("Add a Node to a Graph", () => {
    const g = new Graph<MetaType, NodeType, EdgeType>({ name: "top_level", version: "4.1" });
...
test("Add two nodes to the graph and connect them", () => {
    ...
    if (root) {
        g.traverse(root, (n) => visits.push(valueOrNull<string>(n.property("label") as string, "")));
    }
